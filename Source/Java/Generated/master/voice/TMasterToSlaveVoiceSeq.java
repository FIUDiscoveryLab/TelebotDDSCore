...
/**
 * A sequence of TMasterToSlaveVoice instances.
 */
public final class TMasterToSlaveVoiceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public TMasterToSlaveVoiceSeq() {
        super(TMasterToSlaveVoice.class);
    }


    public TMasterToSlaveVoiceSeq(int initialMaximum) {
        super(TMasterToSlaveVoice.class, initialMaximum);
    }


    public TMasterToSlaveVoiceSeq(Collection elements) {
        super(TMasterToSlaveVoice.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, TMasterToSlaveVoice.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' TMasterToSlaveVoice objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TMasterToSlaveVoice.create());
