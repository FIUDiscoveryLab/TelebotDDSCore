...
/**
 * A sequence of TSlaveToMasterMobilityFeedback instances.
 */
public final class TSlaveToMasterMobilityFeedbackSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public TSlaveToMasterMobilityFeedbackSeq() {
        super(TSlaveToMasterMobilityFeedback.class);
    }


    public TSlaveToMasterMobilityFeedbackSeq(int initialMaximum) {
        super(TSlaveToMasterMobilityFeedback.class, initialMaximum);
    }


    public TSlaveToMasterMobilityFeedbackSeq(Collection elements) {
        super(TSlaveToMasterMobilityFeedback.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, TSlaveToMasterMobilityFeedback.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' TSlaveToMasterMobilityFeedback objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TSlaveToMasterMobilityFeedback.create());
